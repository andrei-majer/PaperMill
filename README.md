...
- Python 3.11+
- A GPU is **strongly recommended** for local embeddings and Florence-2 image ingestion (CUDA on Windows/Linux, Metal/MPS on Apple Silicon)
- [Ollama](https://ollama.com/) running locally (default LLM provider) — or use a cloud provider (Anthropic, OpenAI, OpenRouter)
...
**Best: Windows/Linux x64 with an NVIDIA GPU (≥ 11 GB VRAM).** Dedicated VRAM doesn't compete with system RAM, and CUDA is the most mature accelerator path for both Florence-2 and Jina v3. The LLM can run on CUDA-accelerated Ollama, or you can offload to a cloud provider.

**Recommended for Mac: Apple Silicon M4 Pro with 24 GB unified memory.** Florence-2 (image captioning + OCR) and Jina v3 embeddings run on the integrated GPU via Metal/MPS, and a local LLM (`llama3.1:8b` or `gemma4:e4b`) fits comfortably alongside without swapping.

**Workable but tight: 16 GB unified memory (M-series base).** The full local stack — Florence-2 + Jina embedder + an 8B Ollama model — totals ~20 GB resident, so the OS will swap during image ingestion or generation. Either accept the slowdown or offload generation to a cloud provider (Claude / OpenAI / OpenRouter), which removes the LLM's ~5–10 GB footprint.

**Not recommended: AMD/Intel x64 CPU-only.** Without a GPU, Florence-2 image ingestion runs into the minutes per image and local embeddings drag PDF ingestion to a crawl. The app will still run, but the experience is poor enough that you're better off either adding a GPU or running the whole pipeline on a cloud LLM/embedding provider.

| Component | RAM estimate |
|---|---|
| OS + Streamlit + LanceDB | ~2–3 GB |
| Jina v3 embeddings (local) | ~600 MB |
| Florence-2-large (image ingest) | ~2.5 GB |
...
<details>
<summary><b>Tuning scanner speed</b></summary>

Image and PDF ingestion can stall on the LLM escalation step (the OllamaClassifierBackend issues a blocking 30 s call to `OLLAMA_DRAFT_MODEL` whenever the heuristic suspicion score crosses the threshold). For trusted local content the LLM gate is usually unnecessary — Florence-2's varied prose captions in particular trip the default 0.6 threshold easily. Options, fastest first:

| Fix | Effect | Trade-off |
|---|---|---|
| `SCANNER_LLM_ESCALATION = False` in `config.py` | Skips the LLM call entirely. Regex still runs. | Less malicious-prompt detection — fine for personal screenshots and trusted PDFs |
| Raise `SCANNER_SUSPICION_THRESHOLD` from 0.6 → 0.9 | Only highly suspicious text gets LLM-checked | Quieter, still some coverage |
| Switch scanner model to a smaller LLM (e.g. `llama3.2:3b`) | LLM call 3–5× faster (~5–10 s vs ~30 s) | Need `ollama pull llama3.2:3b` first; less accurate classifier |
| Tighten the 30 s timeout in `OllamaClassifierBackend.scan` | Caps the worst-case stall | Slow real classifications get cut off and skipped (fail-open) |

These are particularly worth doing on a 16 GB Apple Silicon box, where the scanner LLM (`llama3.1:8b`, ~5 GB) competes with Florence-2 (~2.5 GB) and the Jina embedder for unified memory.
